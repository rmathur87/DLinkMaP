...
with open( rerun_script, "w" ) as scfile:
	...
	for run in not_run:
		...
		print( "Made script for run_{0}: {1:04}/{2:04} = {3:.3f}%".format( run, not_run.index( run )+1, len( not_run ), ((not_run.index(run)+1)/len(not_run))*100 ), end="\r" )
		time.sleep(0.01)

# creates a subset of the all_rerun file to automatically run the first 10 models
os.system( "head -702 all_rerun.sh > rerun_100.sh" )
os.system( "chmod +x rerun_100.sh " )
