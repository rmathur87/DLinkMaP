...
### This grabs all 3 froms of X from teh DSPR hap positions.. 
...
### This provides indice matches to turn Full into dom into additive...
...
## the LOD to p converter for output from ap
LODp <- function(out) {
...
QTL.F.Map <- function(QTLI, L.V, LineM, LineF, XNull, XLNull, SS.Null, z, sqrtW=1) {
   ### Set Up X
  n <- length(z)
  XQTL <- makeX(QTLI, LineM, LineF)
  
  Xq <- c(XQTL[1:3], sapply(XQTL[1:3], function(x) XNull[,2] * x))
  names(Xq)[4:6] <- paste0(names(Xq)[4:6], '_D')
  XL <- sapply(Xq, function(x) (solveL(L.V[[1]], x) * sqrtW))
  XXR <- tryCatch(fX.R(XL, XLNull), error = function(e) {print('LaPack!!');NA})
  #XXR <- fX.R(XL, XLNull)
  if(is.na(XXR[1])) return(list(out=rep(NA, 12), log.lik=rep(NA,6)))
  ...
  Ly <- sapply(XXR, function(x) sum(colSums(x * z)^2))
  ...
  log.lik <- (-n/2) * (1+log(2*pi*(SSEm/n)))
  
  list(out=out, log.lik=log.lik)
}

QTL.F.Map.Inter <- function(QTLI, L.V, LineM, LineF, XNull, XLNull, SS.Null, z, interQTL, sqrtW=1) {
